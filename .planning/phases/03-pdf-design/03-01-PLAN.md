...
must_haves:
  truths:
    ...
    - "Alle bestehenden Seiten rendern korrekt mit den neuen Farben — kein visueller Crash"
  artifacts:
...
<task type="auto">
  ...
  - Verify no old green colors remain: `grep -r '#06402b\|#042e1f\|#267e61' src/services/pdf/pages/` returns no matches
  </verify>
  <done>
...
<task type="auto">
  ...
  2. Completely rewrite `src/services/pdf/pages/titlePage.js` for the hero image design:
     ...
     Define `const assetsDir = path.resolve(__dirname, '..', '..', '..', '..', 'assets');` at the top of titlePage.js — same pattern as componentPage.js.
