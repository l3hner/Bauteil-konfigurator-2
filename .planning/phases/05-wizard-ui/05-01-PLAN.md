...
must_haves:
  truths:
    - "Fachberater sieht immer nur einen Schritt -- alle anderen Abschnitte sind ausgeblendet"
    - "Weiter-Taste navigiert zum nachsten Schritt, Zuruck-Taste zum vorherigen Schritt"
    - "Browser-Zuruck-Taste navigiert zwischen besuchten Schritten, URL zeigt #step-N"
    - "Nach Seitenaktualisierung (F5) sind alle Eingaben erhalten und der Wizard kehrt zum letzten Schritt zuruck"
    - "KfW-abhangige Wand- und Luftungsoptionen werden nach Wiederherstellung korrekt angezeigt"
    - "Auf Schritt 17 ist der Absenden-Button sichtbar, auf allen anderen Schritten ausgeblendet"
  artifacts:
...
<task type="auto">
  ...
  **Create `public/js/wizard.js`:**
  ...
  3. **`goToStep(stepNumber)`:**
     ...
     - **Show/hide submit section:** If `stepNumber === 17`, set `document.getElementById('wizard-submit-section').style.display = 'block'`. Otherwise, set it to `display = 'none'`. This ensures the submit button is only visible on the final step.
...
<task type="auto">
  ...
  **Update `views/index.ejs`:**
  ...
  1b. **Add a 17th progress step to the progress bar.** The existing progress bar in index.ejs has 16 `.progress-step` elements (data-step 1 through 16). The wizard defines 17 steps. Add a 17th step after step 16:
     ```html
     <div class="progress-step" data-step="17">
         <div class="progress-step-indicator">17</div>
         <span class="progress-step-label">Berater</span>
     </div>
     ```
     This ensures `updateWizardProgress()` finds a DOM element for every step in the WIZARD_STEPS array.
