...
Do NOT change: the 10 KB skip threshold, the caching logic, the error fallback behavior, or the clearCache function. Keep JPEG quality at 75 (research confirmed this is sufficient).
  ...
  <verify>
1. Verify module loads: `node -e "require('./src/services/imageService')"`

2. Generate a full test PDF by starting the server (`npm start`), submitting a configuration with all components selected, then measure the output file size:

```bash
ls -la output/*.pdf | tail -1
# Expected: file size under 5,242,880 bytes (5 MB)
# If the file is over 5 MB, reduce JPEG quality from 75 to 60 in both branches and re-test
```

3. Check console output during PDF generation for "[Image] Compressed ... (alpha flattened)" lines confirming the 3 large alpha images are being flattened and compressed (should show 80%+ reduction).

4. Verify small images are still skipped: console should show "[Image] Skipping tiny image" for files under 10 KB.
  </verify>
  <done>
imageService.js flattens alpha channels to white before JPEG conversion, uses maxWidth=600 as default, and generated PDFs with full component selection are under 5 MB. If initial quality=75 exceeds 5 MB, quality has been reduced to 60 as fallback.
  </done>
</task>
...
2. Generate a full PDF (all components, all images) and verify file size < 5 MB via `ls -la output/*.pdf`
...
- Generated PDF with all components is under 5 MB (measured via ls -la)
